...
| `DirectiveScopeFilter` | `features/goals/components/directive-scope-filter.tsx` | Client | **Deferred** — removed from the Directives page UI after user testing found the filter labels ("Prestige Ops", "Economics", "Logistics") confusing without explanation. The underlying `directive-scopes.ts` lib is retained for engine use but the UI surface is hidden. Will be re-added when the feature provides clearer value or tooltips are designed. |
| `ProgressionSummary` | `features/goals/components/progression-summary.tsx` | Client | Right-column side panel. Read-only display of overall directive progress (completed/total with percentage and progress bar). Header shows a circle icon + "PROGRESSION SUMMARY" telemetry label. Sub-category breakdown (Prestige Ops / Economic / Logistics) is **deferred** alongside the `DirectiveScopeFilter`. |
...
| `OpenQuestsPreview` | `features/goals/components/open-quests-preview.tsx` | Client | Right-column side panel. Top 3 in-progress quest chains for the active directive with per-chain % completion. When no multi-part series are detected, falls back to showing top actionable individual quests grouped by trader. Empty state differentiates between "not connected" (→ "Connect to view quests") and "connected but no open quests" (→ "All quest chains complete!"). Header has a "!" icon + "OPEN QUESTS" telemetry label and an "ACTIVE DATA" pill. |
...
- [x] Page renders the **TTI Directives layout**: `PageHeader` with "Directives" title + subtitle + `TarkovTrackerProfileLink` primary CTA in the actions slot, **2-column body** (hero column 8/12 with `ActiveDirectiveFocus`, side column 4/12 stacking `ProgressionSummary` → `OpenQuestsPreview`). `DirectiveScopeFilter` is **deferred** (see UI Components).
- [x] On mobile (`<lg`), layout collapses to a single column: `ActiveDirectiveFocus` → `ProgressionSummary` → `OpenQuestsPreview` → goal switcher row
...
- [ ] ~~`DirectiveScopeFilter`~~ — **deferred**, removed from UI. The underlying scope infrastructure in `lib/directive-scopes.ts` is retained.
- [x] `ProgressionSummary` shows a single "Overall" progress bar with completed/total count and percentage for the active directive. Sub-category rows are **deferred** alongside `DirectiveScopeFilter`.
- [x] `OpenQuestsPreview` shows top 3 in-progress task chains (series or trader-grouped). When no multi-part series are detected, falls back to showing top actionable individual quests. Empty state: "Connect to view in-progress quest chains." when disconnected; "No more required quest chains for this directive." when connected with no open quests.
...
- Player has completed all actionable tasks but blocked tasks remain → focus card stays at goal-level percentage; `HardRequirementsList` shows no actionable tasks but progress reflects total scope
